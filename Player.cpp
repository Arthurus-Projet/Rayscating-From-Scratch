...
bool Player::moveForward(int** map) {
    ...
    double pos_y = y - (speed * 8) * std::sin(degToRad(angle));
    double pos_x = x + (speed * 8) * std::cos(degToRad(angle));
    if (map[static_cast<int>(pos_y)][static_cast<int>(pos_x)] == 0)
    {
        setX(x + speed * 4 * std::cos(degToRad(angle)));
        setY(y - speed * 4 * std::sin(degToRad(angle)));
        return true;
    }
    return false;
}
...
bool Player::takeAction(int action, int** map, double XOtherPlayer, double YOtherPlayer, double hyp_reference_angle) {
    if (action == 0)
        moveForward(map);
    if (action == 1)
        moveBack(map);
    ...
    if (action == 3)
        newAngle(-1);
    if (action == 4)
        return shoot(XOtherPlayer, YOtherPlayer, hyp_reference_angle);
    return false;
}
